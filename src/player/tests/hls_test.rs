...
mod tests {
    ...
    async fn test_parse_m3u8_parse_err() {
        ...
        // FutureStream not awaited on
        assert_eq!(0, mock_fn.count(), "get_m3u8_factory was not called");
    }

    #[tokio::test]
    #[ignore = "m3u8-rs not parsing master playlist correctly"]
    async fn test_parse_m3u8_master_playlist_file() {
        match m3u8_rs::parse_playlist_res(stubs::MASTER_PLAYLIST_FILE.as_bytes()) {
            Result::Ok(m3u8_rs::Playlist::MasterPlaylist(pl)) => {
                let mut mock_fn = stubs::MockResolveUriFactoryFn::new();
                mock_fn.set_bytes(stubs::MASTER_PLAYLIST_FILE);
                let future = parse_m3u8_master_playlist(pl, mock_fn.call());
                let outcome = future.await;
                assert!(outcome.is_ok(), "outcome is not ok.");
                assert_eq!(mock_fn.count(), 1, "get_m3u8_factory was not called");
            }
            _ => {
                assert!(false, "error parsing m3u8 master file")
            }
        }
    }
